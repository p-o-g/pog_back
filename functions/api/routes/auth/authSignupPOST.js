  ...
  try {
    ...
    if (userFirebase.err) {
      if (userFirebase.error.code === 'auth/email-already-exists') {
        return res.status(statusCode.CONFLICT).json(util.fail(statusCode.CONFLICT, '이미 가입된 메일입니다.'));
      } else if (userFirebase.error.code === 'auth/invalid-password') {
        return res.status(statusCode.PRECONDITION_FAILED).json(util.fail(statusCode.PRECONDITION_FAILED, '비밀번호 형식이 잘못되었습니다. 패스워드는 최소 6자리의 문자열이어야 합니다.'));
      } else {
